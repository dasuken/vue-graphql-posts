...
module.exports = {
  Query: {
    getUser: async (parent, { query }, { User }) => {
      return User.findOne({ username: query })
    },
    getPosts: (_, args, { Post }) => {
  ...
  Mutation: {
    ...
    updateUserPost: async (_, { title, imageUrl, categories, description, userId, postId }, { Post }) => {
    ...
    signinUser: async(_, { username, password }, { User }) => {
      ...
      const invalidPassword = await bcrypt.compareSync(password, user.password)

      if(!invalidPassword) throw new Error('Password is invalid')
